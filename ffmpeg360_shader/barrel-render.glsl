...
const mediump float Expand_coef = 1.01f;
...
mediump vec2 toBarrel(mediump vec3 ps)
{
    mediump vec3 ps_rotate = rotationMatrix(radians(vec3(-pitch, yaw+180.0f*1.01f, roll))) * ps;
    mediump vec2 st = toSpherical(ps_rotate);

    if (st.y >= M_PI * 0.25f && st.y <= M_PI * 0.75f) {
        // center band
        st.x = st.x / (2.5f * M_PI) / Expand_coef;
        st.y = ((st.y - 0.25f * M_PI) / (0.5f * M_PI) - 0.5f) / Expand_coef + 0.5f;
        return st;
    }
    else if (st.y < M_PI * 0.25f) {
        // top band
        mediump vec3 ps_rotate = rotationMatrix(radians(vec3(-pitch, yaw+90.0f, roll))) * ps;
        st.x = -ps_rotate.z / ps_rotate.y * 0.1f / Expand_coef + 0.9f;
        st.y = ps_rotate.x / (ps_rotate.y * 0.4f) * 0.1f / Expand_coef + 0.25f;
        return st;
    }
    else {
        // bottom band
        mediump vec3 ps_rotate = rotationMatrix(radians(vec3(-pitch, yaw+90.0f, roll))) * ps;
        st.x = (ps_rotate.z / ps_rotate.y * 0.1f) / Expand_coef + 0.9f;
        st.y = (ps_rotate.x / (ps_rotate.y * 0.4f) * 0.1) / Expand_coef + 0.75f;
        return st;
    }
    return vec2(0.0);
}
...
void main(void)
{
    ...
    mediump vec2 textureCoord = toBarrel(toCartesian(sphericalCoord));
